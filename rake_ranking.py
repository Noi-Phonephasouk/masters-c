...
for row in rows:
	if row[4] not in sim_data_cat:
		sim_data_cat.append(int(row[4]))
...
for j in range(0,len(sim_data_cat)):
	for i in range(0, len(sim_sentls_set)):
		if sim_data_cat[j] == sim_sentls_set[i]['classid']:
			lst_sent_classid.append(sim_sentls_set[i]['review_sent'])
			sim_data.append(sim_sentls_set[i]['review_sent'])
	
	phraseList = generate_candidate_keywords(lst_sent_classid,stoppath)
	...
	print "Category ", sim_data_cat[j]
	
	for keyword in sortedKeywords[0:5]:
		print "Keyword" , str(sortedKeywords.index(keyword)+1)+ ": " + keyword[0], ", score: ", keyword[1]

	lst_sent_classid=[]
	print

phraseList = generate_candidate_keywords(sim_data,stoppath)
word_score = calculate_word_scores(phraseList)
keywordcandidates = generate_candidate_keyword_scores(phraseList,word_score)
